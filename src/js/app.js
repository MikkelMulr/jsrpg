...
class player {
  ...
  updateStats = () => {
    document.querySelector('.player--stats--HP').innerHTML = `HP: <span>${newPlayer.player_init_HP}</span>`;
    document.querySelector('.player--stats--MP').innerHTML = `MP: <span>${newPlayer.player_init_MP}</span>`;

    document.querySelector('.monster--stats--HP').innerHTML = `HP: <span>${newMonster.monster_HP}</span>`;
    document.querySelector('.monster--stats--MP').innerHTML = `MP: <span>${newMonster.monster_MP}</span>`;
  };
  ...
  init_player = (name, job) => {
    ...
    if (this.playerJob in this.jobs) {
      if (this.playerJob === 'Assassin') {
        ...
        img.style.backgroundImage = `url('./src/assets/img/icons/sacrificial-dagger.png')`;
        ...
        img.style.backgroundImage = `url('./src/assets/img/icons/orb-wand.png')`;
        ...
        img.style.backgroundImage = `url('./src/assets/img/icons/sharp-axe.png')`;
        ...
        img.style.backgroundImage = `url('./src/assets/img/icons/pocket-bow.png')`;
        ...
        img.style.backgroundImage = `url('./src/assets/img/icons/winged-sword.png')`;
      }
    }
...
class monster {
  ...
  init_monster = (rando) => {
    ...
    if (rando <= 0.25) {
      ...
      this.monster_type = this.monster_types[3];
      this.monster_HP = 165;
      this.monster_MP = 100;
...
player_attack = () => {
  let dmg = 0;
  let ui = document.querySelector('.ui-events--list');
  if (Math.random() < newPlayer.player_crit_chance) {
    dmg = Math.floor(newPlayer.playerATTK * newPlayer.player_crit_chance + newPlayer.playerATTK);
  } else {
    dmg = newPlayer.playerATTK;
  }
  dmg > newPlayer.playerATTK ? newGame.eventList.push('CRITICAL HIT!') : null; // display if crit dmg
  newGame.eventList.push(`${newPlayer.playerName} attacks for ${dmg} points of dmg`);
  newMonster.monster_HP -= dmg; // monster takes dmg

  if (newMonster.monster_HP > 0) {
  ...
  if (newMonster.monster_HP <= 0) {
    ui.innerHTML += `<li class="ui-events--list--item">The ${newMonster.monster_type} has been defeated!</li>`;
    ui.innerHTML += `<li class="ui-events--list--item">But this is no time to rest, something else approaches from the darkness...</li>`;

    newMonster.init_monster(Math.random());
    ui.innerHTML += `<li class="ui-events--list--item">A ${newMonster.monster_type} appeared! (Choose an action)</li>`;
  }
};
...
monster_attack = () => {
  let dmg = 0;
  let ui = document.querySelector('.ui-events--list');
  if (Math.random() < newMonster.monster_crit) {
    dmg = Math.floor(newMonster.monster_ATTK * newMonster.monster_crit + newMonster.monster_ATTK);
  } else {
    dmg = newMonster.monster_ATTK;
  }
  dmg > newMonster.monster_ATTK ? newGame.eventList.push('CRITICAL HIT!') : null; // display if crit dmg
  newGame.eventList.push(`${newMonster.monster_type} attacks for ${dmg} points of dmg`);
  newPlayer.player_init_HP -= dmg; // player takes dmg

  if (newPlayer.player_init_HP > 0) {
    newPlayer.updateStats();
    newGame.updateEventList();
  }
  ...
  if (newPlayer.player_init_HP <= 0) {
    ...
    ui.innerHTML += `<li class="ui-events--list--item">${newPlayer.playerName} has been defeated!</li>`;
    ui.innerHTML += `<li class="ui-events--list--item">The darkness was clearly too much for you this time...</li>`;

    ui.innerHTML += `<li class="ui-events--list--item">GAME OVER</li>`;
  }
};
